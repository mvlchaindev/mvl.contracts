...
#include <mvl.token/mvl.token.hpp>

//#include <mvllib/token.hpp>
...
namespace mvl {
  ...
  struct [[eosio::table, eosio::contract("mvl.system")]] user_resources {
    name          owner;
    asset         net_weight;
    asset         cpu_weight;
    int64_t       ram_bytes = 0;
  ...
  struct [[eosio::table, eosio::contract("mvl.system")]] delegated_bandwidth {
    name          from;
    name          to;
    asset         net_weight;
    asset         cpu_weight;
  ...
  struct [[eosio::table, eosio::contract("mvl.system")]] refund_request {
    name            owner;
  ...
  void system_contract::buyram( name payer, name receiver, asset quant ) {
    ...
    INLINE_ACTION_SENDER(mvl::token, transfer)(
  ...
  void system_contract::sellram( name account, int64_t bytes ) {
    ...
    _gstate.total_ram_bytes_reserved -= static_cast<decltype(_gstate.total_ram_bytes_reserved)>(bytes); // bytes > 0 is asserted above
      ...
      INLINE_ACTION_SENDER(mvl::token, transfer)(
          token_account, { {ram_account, active_permission}, {account, active_permission} }, { ram_account, account, asset(tokens_out), std::string("sell ram") }
      );
  ...
  void system_contract::changebw( name from, name receiver,
    ...
    check( std::abs( (stake_net_delta + stake_cpu_delta).amount ) >= std::max( std::abs( stake_net_delta.amount ), std::abs( stake_cpu_delta.amount ) ), "net and cpu deltas cannot be opposite signs" );
  ...
  void system_contract::delegatebw( name from, name receiver, asset stake_net_quantity, asset stake_cpu_quantity, bool transfer ) {
  ...
  void system_contract::undelegatebw( name from, name receiver,asset unstake_net_quantity, asset unstake_cpu_quantity ) {
    asset zero_asset( 0, core_symbol() );
  ...
  void system_contract::refund( const name owner ) {
    ...
    INLINE_ACTION_SENDER(mvl::token, transfer)(
        token_account, {{stake_account, active_permission}, {req->owner, active_permission}}, {stake_account, req->owner, req->net_amount + req->cpu_amount, std::string("unstake")}
    );
